...
from slog.ende_zu_ende.keypointnet_informed_don import KeypointNetInformedDON
...
if __name__ == "__main__":
    ...
    for j in tqdm(range(3), desc="Benchmarking AUC of PCK@k", total=3):
        ...
        for k in tqdm(range(1, 100), desc=f"Iteration ~ {j}", total=99):
            ...
            descriptor_a = model._forward_don(rgb_a.permute(2, 0, 1).unsqueeze(0)).squeeze(dim=0).to(device)
            ...
            descriptor_b = model._forward_don(rgb_b.permute(2, 0, 1).unsqueeze(0)).squeeze(dim=0).to(device)
